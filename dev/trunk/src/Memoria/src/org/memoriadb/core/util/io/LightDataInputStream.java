 ...
 * input stream.
 * <p>
 * DataInputStream is not necessarily safe for multithreaded access. Thread safety is optional and is the responsibility
 * of users of methods in this class.
 * 
 ...
 * @since JDK1.0
 */
public class LightDataInputStream extends DataInputStream implements IDataInput {
  
//  private static byte[] bytearr = new byte[1000];
//  private static char[] chararr = new char[1000];
//  private static char[] lineBuffer = new char[1000];
//  private static byte[] longBuffer = new byte[8];
//
//  /**
//   * Reads from the stream <code>in</code> a representation of a Unicode character string encoded in <a
//   * href="DataInput.html#modified-utf-8">modified UTF-8</a> format; this string of characters is then returned as a
//   * <code>String</code>. The details of the modified UTF-8 representation are exactly the same as for the
...
//        if (((char2 & 0xC0) != 0x80) || ((char3 & 0xC0) != 0x80)) throw new UTFDataFormatException("malformed input around byte "
//            + (count - 1));
...
//
   ...
   */
  public LightDataInputStream(InputStream in) {
    super(in);
  }
//
//  /**
//   * Reads some number of bytes from the contained input stream and stores them into the buffer array <code>b</code>.
//   * The number of bytes actually read is returned as an integer. This method blocks until input data is available, end
//   * of file is detected, or an exception is thrown.
//   * 
//   * <p>
//   * If <code>b</code> is null, a <code>NullPointerException</code> is thrown. If the length of <code>b</code> is zero,
//   * then no bytes are read and <code>0</code> is returned; otherwise, there is an attempt to read at least one byte. If
//   * no byte is available because the stream is at end of file, the value <code>-1</code> is returned; otherwise, at
//   * least one byte is read and stored into <code>b</code>.
//   * 
//   * <p>
//   * The first byte read is stored into element <code>b[0]</code>, the next one into <code>b[1]</code>, and so on. The
//   * number of bytes read is, at most, equal to the length of <code>b</code>. Let <code>k</code> be the number of bytes
//   * actually read; these bytes will be stored in elements <code>b[0]</code> through <code>b[k-1]</code>, leaving
//   * elements <code>b[k]</code> through <code>b[b.length-1]</code> unaffected.
//   * 
//   * <p>
...
//   * @exception IOException
//   *              if the first byte cannot be read for any reason other than end of file, the stream has been closed and
//   *              the underlying input stream does not support reading after close, or another I/O error occurs.
...
//   * 
//   * <p>
//   * If <code>len</code> is zero, then no bytes are read and <code>0</code> is returned; otherwise, there is an attempt
...
//   * 
//   * <p>
//   * The first byte read is stored into element <code>b[off]</code>, the next one into <code>b[off+1]</code>, and so on.
//   * The number of bytes read is, at most, equal to <code>len</code>. Let <i>k</i> be the number of bytes actually read;
//   * these bytes will be stored in elements <code>b[off]</code> through <code>b[off+</code><i>k</i><code>-1]</code>,
//   * leaving elements <code>b[off+</code><i>k</i><code>]</code> through <code>b[off+len-1]</code> unaffected.
//   * 
//   * <p>
//   * In every case, elements <code>b[0]</code> through <code>b[off]</code> and elements <code>b[off+len]</code> through
...
//   *             read lines of text is via the <code>BufferedReader.readLine()</code> method. Programs that use the
...
//   * @exception IOException
//   *              if the contained input stream does not support seek, or the stream has been closed and the contained
//   *              input stream does not support reading after close, or another I/O error occurs.
//   */
